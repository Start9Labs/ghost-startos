...
This repo packages [Ghost](https://ghost.org/) for StartOS.
...
Ghost runs from the official `ghost` Alpine image on Docker Hub, and its database from the official `mysql` image. To track a new upstream release of either:

1. Bump the relevant `dockerTag` in `startos/manifest/index.ts`:
   - Ghost: `ghost:<new version>-alpine`
   - MySQL: `mysql:<new version>` (bump only when intentionally moving MySQL — keep major versions aligned with what Ghost supports).
...
3. Rebuild (`make`), sideload the `.s9pk`, and confirm it starts. MySQL's first-run initialization on a fresh install can take several minutes; allow for it.
